"""
Train T3 per-tuple tree model on TPC-H SF1 extended (augmented) PostgreSQL plans.

Uses 16 examples for training (seed 42 shuffling), rest for test. Saves to
model_pg_extended.txt. Plans are loaded from tpch_sf1/plans/extended/ and
converted with pg_to_umbra (ground-truth: actual_scan_in_card, component_selectivity, ius).

Requires: lightgbm, sklearn, and project deps. Database: tpchSf1.

Usage (from T3 project root):
  python -m src.postgres.training_tpch_extended
  python -m src.postgres.training_tpch_extended --train-n 8 --out my_model.txt
"""
...
# TPC-H SF1 extended plans directory (next to this file)
TPCH_SF1_EXTENDED_DIR = Path(__file__).resolve().parent / "tpch_sf1" / "plans" / "extended"

SEED = 42
TRAIN_N = 16
DEFAULT_MODEL_PATH = "model_pg_extended.txt"
DB_NAME = "tpchSf1"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train T3 on TPC-H SF1 extended plans (16 train, rest test, seed 42)."
    )
    parser.add_argument(
        "--data",
        type=Path,
        default=TPCH_SF1_EXTENDED_DIR,
    ...
    parser.add_argument(
        ...
        type=int,
        default=TRAIN_N,
        help=f"Number of examples for training (default: {TRAIN_N})",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    n_train = min(args.train_n, len(shuffled))
