"""
Train T3 on zero-shot parsed plans with imdb held out as test set.

Same as training_zeroshot_tpch_holdout but with imdb as the holdout and
default output model_zero_holdout_imdb.txt. If that file already exists,
saves to model_zero_holdout_imdb_v1.txt, then _v2, etc. (next free number).

Usage (from T3 project root):
  python -m src.zeroshot.training_zeroshot_imdb_holdout
  python -m src.zeroshot.training_zeroshot_imdb_holdout --data /path/to/parsed_plans
"""
...
HOLDOUT_IMDB = "imdb"
DEFAULT_MODEL_PATH = "model_zero_holdout_imdb.txt"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train T3 on zero-shot parsed plans with imdb held out as test set."
    )
    parser.add_argument(
        "--data",
        type=Path,
    ...
    train_paths, test_paths = split_train_test_by_holdout(
        all_json_paths, holdout_name=HOLDOUT_IMDB
    )

    if not train_paths:
        print(f"Error: no train files (all paths contain '{HOLDOUT_IMDB}').")
        sys.exit(1)

    print(f"JSON files: {len(all_json_paths)} total")
    print(f"Train (all except {HOLDOUT_IMDB}): {len(train_paths)} files")
    print(f"Test ({HOLDOUT_IMDB}): {len(test_paths)} files")
    ...
    if not args.no_eval and test_paths:
        ...
        if test_queries:
            ...
            summary = (
                f"Test set ({HOLDOUT_IMDB}, {len(test_queries)} queries): "
        ...
        print(f"No test files (no path contains '{HOLDOUT_IMDB}').")
