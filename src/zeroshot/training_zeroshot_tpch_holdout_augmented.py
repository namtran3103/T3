"""
Train T3 on zero-shot DeepDB-augmented plans with one benchmark held out as test set.

Same as training_zeroshot_tpch_holdout but:
- Data from runs/deepdb_augmented/ (parsed + DeepDB cardinalities).
- Conversion via augmented_zeroshot_to_t3 (uses dd_est_card when present).
- Output model named with _augmented; results appended to holdout_augmented.txt.

Usage (from T3 project root):
  python -m src.zeroshot.training_zeroshot_tpch_holdout_augmented
  python -m src.zeroshot.training_zeroshot_tpch_holdout_augmented --data /path/to/deepdb_augmented --out model_zero_holdout_tpc_h_augmented.txt
"""
...
from src.zeroshot.augmented_zeroshot_to_t3 import (
...
DEFAULT_DATA_DIR = "/Users/namtran/Downloads/zero-shot-data/runs/deepdb_augmented"
DEFAULT_MODEL_PATH = "model_zero_tpch_holdout_augmented.txt"
HOLDOUT_TXT = "holdout_augmented.txt"
...
def load_benchmarked_queries_from_zeroshot(
    ...
    """Build BenchmarkedQuery list from DeepDB-augmented zero-shot JSON paths."""
...
def train_per_tuple_model(
    ...
    """Train per-tuple tree model on pipeline feature vectors (PG features for zeroshot)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train T3 on zero-shot DeepDB-augmented plans with one benchmark held out."
    )
    parser.add_argument(
        "--data",
        type=Path,
        default=Path(DEFAULT_DATA_DIR),
        help=f"Root directory containing deepdb_augmented JSON files (default: {DEFAULT_DATA_DIR})",
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    if not args.no_eval and test_paths:
        ...
        if test_queries:
            ...
            print(summary)
            holdout_path = _repo / HOLDOUT_TXT
