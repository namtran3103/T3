"""
Reproduce query-level holdout results for all benchmarks.

For each holdout in HOLDOUTS, trains a query-level model (single summed feature vector per
query) on all other benchmarks and evaluates on the holdout.
...
Usage (from T3 project root):
  python results/models/0_reproduction/query/reproduce.py
"""
...
from src.zeroshot.training_zeroshot_tpch_holdout_ql import (
    estimate_runtime_query_level,
...
def run_holdout(
    ...
    bst = train_query_level_lightgbm(train_queries, seed=SEED)
...
def main() -> None:
    ...
    for i, holdout in enumerate(HOLDOUTS):
        ...
        run_holdout(holdout, act_dir, all_json_paths, use_actual_card=True, feature_mapper=feature_mapper)
        print(f"\n[{i * 2 + 2}/{total}] est — {holdout}")
        run_holdout(holdout, est_dir, all_json_paths, use_actual_card=False, feature_mapper=feature_mapper)
