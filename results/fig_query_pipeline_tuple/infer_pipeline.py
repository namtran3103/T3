"""
Pipeline-level inference for the query/pipeline/tuple comparison figure.

Uses act models from results/models/pipeline-level/act/ with actual cardinalities.
...
Results are appended to pipeline.txt in this directory.
"""
...
MODEL_DIR = SCRIPT_DIR.parent / "models" / "pipeline-level" / "act"
OUT_FILE = SCRIPT_DIR / "pipeline.txt"
...
def main() -> None:
    ...
    summary = (
        f"pipeline-level (act): {total_queries} queries across {len(model_files)} holdouts | "
